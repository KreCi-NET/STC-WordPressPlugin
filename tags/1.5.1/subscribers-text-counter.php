...
function counters_subscribers_text_counter( $options, $refresh = '0' ) {
	...
	if ( $time <= 0 || $refresh ) {
		if ( !empty( $options['feedburner'] ) || $new ) {
		...
		if ( ( !empty( $options['facebookk'] ) && !empty( $options['facebooks'] ) && !empty( $options['facebook'] ) ) || $new ) {
