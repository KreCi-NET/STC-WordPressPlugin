...
function counters_subscribers_text_counter( $options, $refresh = '0' ) {
	...
	if ( $time <= 0 || $refresh ) {
		...
		if ( !empty( $options['twitter'] ) ) {
			$twitter = followers_count( $options['twitter'], $options['twitterk'], $options['twitters'] );
...
function followers_count( $twitter_id, $consumer_key, $consumer_secret ) {
    // some variables
    $token = get_option('stcTwitterToken');
    ...
    if($twitter_id && $consumer_key && $consumer_secret) {
	    if(!$token) {
	        ...
	        if($keys) {
	            ...
	            update_option('stcTwitterToken', $keys->access_token);
...
?>
	...
	<p>
		...
		<p>
				...
				<ol>
					<li>%twitter%, %facebook%, %youtube%, %wordpress% and %all% to display counters</li>
					<li>%twitterlink%, %facebooklink% and %youtubelink% to insert links</li>
				<ol>
			</p>
		</small>
	</p>
